...
public class Util {
    ...
    public static JsonObject gameReviewsToJson(Document doc) {
        ...
        List<Document> list = doc.getList("reviews", Document.class, List.of());
        ...
        // get timestamp of either latest review
        String timestamp;
        if (list.isEmpty()) {
            timestamp = doc.getDate(REVIEWS_FIELD_POSTED).toString();
        } else {
            Date date = list.get(list.size() - 1).getDate(REVIEWS_FIELD_POSTED);
            timestamp = date.toString();
        }
        // add the last 2 objects
        jsonObjBldr.add("reviews", jsonArrBldr);
        jsonObjBldr.add("timestamp", timestamp.toString());
    ...
    public static JsonObject gameHighestOrLowestRatingToJson(List<Document> listDocs, String direction) {
        ...
        listDocs.stream().forEach(x -> {
            JsonObjectBuilder jsonObjBldr = Json.createObjectBuilder()
                    .add("_id", x.getInteger(REVIEWS_FIELD_OBJECT_ID))
